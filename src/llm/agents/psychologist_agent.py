...
class PsychologistAgent:
    ...
    def __init__(self):
        ...
        self.system_message = SystemMessage(
            content=(
                ...
                "ОБЯЗАТЕЛЬНО используй предоставленный ниже контекст как основу для своего ответа. "
                "Если в контексте содержится конкретная информация (например, адрес, контакты, правила), "
                "ты должен ее упомянуть. Если в контексте нет ответа на вопрос, ты должен прямо сказать: 'К сожалению, у меня нет этой информации в базе знаний'."
            )
        )
    ...
    def answer(self, question: str) -> str:
        ...
        # !!! ВРЕМЕННОЕ ДОБАВЛЕНИЕ ДЛЯ ДИАГНОСТИКИ !!!
        print("\n*** НАЧАЛО ИЗВЛЕЧЕННОГО КОНТЕКСТА (K=7) ***")
        if not contexts:
            print("ВНИМАНИЕ: VectorStore вернул пустой список контекстов.")
        for i, context in enumerate(contexts):
            print(f"[{i+1}]: {context}")
        print("*** КОНЕЦ ИЗВЛЕЧЕННОГО КОНТЕКСТА ***\n")
